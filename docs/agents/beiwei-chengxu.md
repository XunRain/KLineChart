...
- “收获全部成熟作物”开发者商品已接入服务端权威报价、部分容量通用确认、持久 intent、迟到收据 credit 与 Schema v9 幂等收据结算；客户端只绑定 `GrowingList.Exit.HarvestAllButton` 并回传短期 quote token，按钮本体仍由工具人在 Studio 创建和排版。
...
- `RequestSeedShopRestock` 无参数请求购买一次个人种子商店刷新；服务端必须重新校验玩家数据、商店距离和商品请求冷却，测试直发或正式收据会同事务保存该玩家刷新代次、绝对到期时间与当前剩余库存快照，重连只恢复余量且绝不改动其他玩家库存。
...
- 当前 DataStore 名集中在 `server/Config/DataStoreConfig.luau`，不得随意修改；未来迁移必须递增 Schema 版本并保持旧数据兼容。种子 51 档、成长跳过 2 档、个人刷新、永久容量和收获全部正式开发者商品 ID 已集中接入对应 IAP 配置，未经工具人提供的新后台配置不得自行更改或发明产品 ID。
